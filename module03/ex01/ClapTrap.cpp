...
ClapTrap::ClapTrap(const ClapTrap &other)
{
	...
	std::cout << YELLOW << "Copy constructor of ClapTrap called (another " << PINK << _name << GREEN << " created" << YELLOW << ").";
	std::cout << RESET << std::endl;
}

ClapTrap &ClapTrap::operator=(const ClapTrap &other)
{
	std::cout << BLUE << "Copy assignment operator of ClapTrap called (" << PINK << _name << BLUE << " changed his name to ";
...
void	ClapTrap::takeDamage(unsigned int amount)
{
	...
	{
		_hit_points = 0;
		std::cout << PINK << _name << RED << " was killed " << WHITE << "(hit points = " << RED << 0 << WHITE << ")";
		std::cout << RESET << std::endl;
		return ;
	}
	_hit_points -= amount;
...
void	ClapTrap::beRepaired(unsigned int amount)
{
	...
	if (_energy_points == 0)
	{
		std::cout << PINK << _name << RED << " cannot repair itself, ";
